...
-- !UP
CREATE TABLE IF NOT EXISTS public.hydra_oauth2_code
(
    ...
    CONSTRAINT hydra_oauth2_code_pkey PRIMARY KEY (signature),
    CONSTRAINT hydra_oauth2_code_challenge_id_fk FOREIGN KEY (challenge_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE CASCADE,
    CONSTRAINT hydra_oauth2_code_client_id_fk FOREIGN KEY (client_id)
        REFERENCES public.hydra_client (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE CASCADE
);

CREATE INDEX hydra_oauth2_code_challenge_id_idx
    ON public.hydra_oauth2_code USING btree
    (challenge_id ASC NULLS LAST)
    TABLESPACE pg_default;

CREATE INDEX hydra_oauth2_code_client_id_idx
    ON public.hydra_oauth2_code USING btree
    (client_id ASC NULLS LAST)
    TABLESPACE pg_default;

CREATE INDEX hydra_oauth2_code_request_id_idx
    ON public.hydra_oauth2_code USING btree
