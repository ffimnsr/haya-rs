...
impl<'a> OauthRefreshToken {
    ...
    pub async fn save_refresh_token(&self, pool: &Pool) -> GenericResult<()> {
    ...
    pub async fn get_refresh_token(pool: &Pool, jwt_id: &Uuid) -> GenericResult<Self> {
        let db = pool.get().await.map_err(|e| e.to_string())?;
