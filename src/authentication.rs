...
pub(crate) struct AuthenticationRequest {
    ...
    /// values used that are not understood by an implementation SHOULD
    /// be ignored. See Sections 5.4 and 11 for additional scope values
    ...
    /// pre-registered at the OpenID Provider, with the matching performed
    /// as described in Section 6.2.1 of (Simple String Comparison). When
    /// using this flow, the Redirection URI SHOULD use the https scheme;
    /// however, it MAY use the http scheme, provided that the Client Type
    /// is confidential, as defined in Section 2.1 of OAuth 2.0, and provided
    /// the OP allows the use of http Redirection URIs in this case. The
